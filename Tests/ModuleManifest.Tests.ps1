...
Describe "ModuleManifest" {
    Context "ModuleManifest" -Tag 'ModuleManifest' {
        Beforeall {
            $manifest = Test-ModuleManifest -Path "$PSScriptRoot/../src/AzClassicModule/AzClassicModule.psd1"
            $functionFiles = Get-ChildItem -Path "$PSScriptRoot/../src/AzClassicModule" -Filter '*.ps1' | Select-Object -ExpandProperty 'BaseName'
        }
